...
public abstract class BaseStatisticsActivity extends BaseActivity {
    ...
    protected AsyncHttpResponseHandler mResponseHandler = new AsyncHttpResponseHandler() {
        ...
        public void onStart() {
            super.onStart();
            if(!isFinishing()){
                DialogHelper.showDialogForLoading(BaseStatisticsActivity.this, "正在加载数据，请您耐心等待...", true);
            }
        }
        ...
        public void onSuccess(int statusCode, Header[] headers, byte[] bytes) {
            ...
            if (200 == statusCode) {
                if (JsonUtil.getStateFromServer(result).equals(Constant.SUCCESS)){
                    ...
                    AppToast.showShortText(BaseStatisticsActivity.this, "获取数据错误");
                }
            }
        ...
        public void onFailure(int arg0, Header[] arg1, byte[] responseBody,
            ...
            if(result.contains("Mismatched access token") || result.contains("X-Access-Token")) {
                ...
                AppToast.showShortText(BaseStatisticsActivity.this, "获取数据失败，请稍后重试");
            }
        }
